...
class PathfinderPDF(FPDF):
    def footer(self):
        ...
        self.cell(
            0,
            5,
            f"Pathfinder AI - Generated {now.month}/{now.day}/{now.year} - Timing estimates may vary Page {self.page_no()} of {{nb}}",
            align="C",
        )
...
def generate_itinerary_pdf(payload: Dict[str, Any]) -> tuple[str, str]:
    ...
    pdf.rect(left_margin, 10, page_width, 45, "F")
    ...
    pdf.cell(page_width, 5, f"PATHFINDER_v1.0.21", ln=True)
    pdf.set_xy(left_margin, 20)
    pdf.cell(page_width, 5, f"ID: {itinerary_id}", ln=True)
    
    pdf.set_font("Arial", "B", 14)
    pdf.set_xy(left_margin, 25)
    pdf.cell(page_width, 8, "EXPEDITION PLAN", ln=True)
    
    pdf.set_font("Arial", "B", 11)
    pdf.set_xy(left_margin, 33)
    pdf.cell(page_width, 6, f"CATANDUANES, PH // HUB: {start_point}", ln=True)
    
    pdf.set_font("Arial", "", 9)
    if start_date and end_date:
        pdf.set_xy(left_margin, 39)
        pdf.cell(page_width, 5, f"{start_date} - {end_date}", ln=True)
    
    pdf.set_xy(left_margin, 44)
    pdf.cell(page_width, 5, f"{day_count} Days - {total_stops} Stops")

    pdf.set_xy(left_margin, 49)
    pdf.cell(page_width, 5, "GENERATED BY PATHFINDER AI", ln=True)
    ...
    for day_idx, day_key in enumerate(sorted_days):
        ...
        pdf.rect(left_margin, pdf.get_y() + 2, page_width, 28, "F")
        pdf.set_xy(left_margin, pdf.get_y() + 9)
        pdf.set_font("Arial", "B", 10)
        pdf.cell(page_width, 5, "MAP SCREENSHOT PLACEHOLDER", align="C", ln=True)
        pdf.set_font("Arial", "", 8)
        pdf.cell(page_width, 5, "Click map image for directions.", align="C", ln=True)
        pdf.ln(18)
        pdf.ln(3)
